...
use crate::local_access::{
    ...
    LocalTypeKind, LocalValueLayout, LocalVariantConstructThunks, LocalVariantProjectIntoThunks,
    LocalVariantProjectThunks, rust_facet_descriptor_for,
};
...
// r[verify binette.compat.enum.payload]
#[test]
fn rust_facet_descriptor_drives_strict_local_enum_encode_stencil() {
    let value = RustLocalEvent::Named {
        code: 0x1122,
        flag: true,
    };
    let plan = writer_plan_for::<RustLocalEvent>().unwrap();
    let descriptor = rust_facet_descriptor_for::<RustLocalEvent>().unwrap();
    let encoder = strict_local_stencil_encoder_from_plan(&plan, &descriptor).unwrap();
    ...
    assert!(encoder.report().helper_paths.is_empty());
    let actual =
        unsafe { encoder.encode_raw_to_vec((&value as *const RustLocalEvent).cast()) }.unwrap();
    assert_eq!(actual, encode_to_vec_with_plan(&value, &plan).unwrap());
}
...
#[test]
fn rust_facet_descriptor_drives_strict_local_string_encode_stencil() {
    let value = "direct local string".to_owned();
    let plan = writer_plan_for::<String>().unwrap();
    let descriptor = rust_facet_descriptor_for::<String>().unwrap();
    let encoder = strict_local_stencil_encoder_from_plan(&plan, &descriptor).unwrap();
    ...
    assert_eq!(encoder.report().helper_count, 0);
    assert!(encoder.report().helper_paths.is_empty());
    let actual = unsafe { encoder.encode_raw_to_vec((&value as *const String).cast()) }.unwrap();
    assert_eq!(actual, encode_to_vec_with_plan(&value, &plan).unwrap());
}
...
fn hybrid_local_encode_stencil_uses_bound_backend_thunks_for_enum_subtree() {
    let values = [
        ...
        SwiftEventEnvelope {
            id: 0x3132_3334_3536_3738,
            event: SwiftProbeEvent::Record(SwiftText {
                id: 0x4142_4344_4546_4748,
                title: "aggregate payload".to_owned(),
                code: 0x7788,
            }),
            code: 0x99AA,
        },
...
fn swift_text_descriptor(root: &TypeRef) -> LocalTypeDescriptor {
    LocalTypeDescriptor::from_import(swift_text_import(root)).unwrap()
}

fn swift_text_import(root: &TypeRef) -> LocalDescriptorImport {
        ...
        kind: LocalDescriptorImportKind::Enum {
            ...
            variants: vec![
                ...
                crate::local_access::LocalVariantImport {
                    ...
                    index: 3,
                    access: LocalAccess::Thunk(swift_event_project_record_thunk()),
...
fn swift_string_thunk_bindings() -> LocalThunkBindings {
    LocalThunkBindings::new()
        ...
        .with_variant_project(
            swift_event_project_record_thunk(),
            LocalVariantProjectThunks {
                project: test_swift_event_project_record,
                context: 0,
            },
        )
...
fn swift_event_project_record_thunk() -> LocalThunk {
...
unsafe extern "C" fn test_swift_event_project_record(
    value: *const u8,
    _context: *mut std::ffi::c_void,
) -> *const u8 {
    match unsafe { &*value.cast::<SwiftProbeEvent>() } {
...
unsafe extern "C" fn test_swift_event_project_record_into(
    value: *const u8,
    out: *mut u8,
    out_len: usize,
    _context: *mut std::ffi::c_void,
) -> bool {
    if out_len != std::mem::size_of::<SwiftText>() {
        return false;
    }
    let SwiftProbeEvent::Record(record) = (unsafe { &*value.cast::<SwiftProbeEvent>() }) else {
        return false;
    };
    unsafe {
        out.cast::<SwiftText>().write(SwiftText {
            id: record.id,
...
unsafe extern "C" fn test_swift_event_construct_record(
    value: *mut u8,
    payload: *const u8,
    payload_len: usize,
    _context: *mut std::ffi::c_void,
) -> bool {
    if payload_len != std::mem::size_of::<SwiftText>() {
        return false;
    }
    let record = unsafe { payload.cast::<SwiftText>().read() };
    unsafe {
        value
            .cast::<SwiftProbeEvent>()
